...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (heroMove && heroIntro) {
        ...
        setTimeout(function() {
            ...
            // 手机端旋转 210 度，桌面端 180 度
            if (cubeRotator) {
                var isMobile = window.matchMedia('(max-width: 767px)').matches;
                var rotateDeg = isMobile ? -210 : -180;
                cubeRotator.style.transform = 'rotateY(' + rotateDeg + 'deg)';
            }
    ...
    // For works page, use the last section (works-section-second)；手机端不设置 footer 定位，由 CSS 流式布局使 footer 出现在内容下方蓝框位置
    ...
    if (worksPage) {
        ...
        if (lastSection) {
            function updateWorksFooter() {
                if (window.innerWidth <= 768) {
                    worksPage.style.minHeight = '';
                    const footer = worksPage.querySelector('.footer');
                    if (footer) footer.style.top = '';
                    return;
                }
                const contentBottom = lastSection.offsetTop + lastSection.offsetHeight;
                const pageHeight = contentBottom + 120;
                worksPage.style.minHeight = pageHeight + 'px';
                const footer = worksPage.querySelector('.footer');
                if (footer) footer.style.top = (pageHeight - 72 - 20) + 'px';
            }
            updateWorksFooter();
            window.addEventListener('resize', updateWorksFooter);
        }
    }
            ...
            if (skillSection) {
                ...
                if (lastContentText) {
                    function updateExperiencePage() {
                        if (window.innerWidth <= 768) {
                            experiencePage.style.minHeight = '';
